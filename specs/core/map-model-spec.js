...
describe('MapModel', function () {
	...
	describe('can use a promise-based layout calculator', () => {
		let anIdea,
			layoutBefore,
			layoutAfter,
		...
		beforeEach(function () {
			layoutCalculator = function () {
				return Promise.resolve(layoutCalculatorLayout);
			};
			layoutBefore = {
				nodes: {
					1: {
		...
		it('initialises from a promise-based layout', done => {
			underTest.setIdea(anIdea).then(() => {
				expect(mapViewResetRequestedListener).toHaveBeenCalled();
			}).then(done);
		});

		it('processes the promise-based layout change', done => {
			const nodeCreatedListener = jasmine.createSpy();

			underTest.setIdea(anIdea).then(() => {
				underTest.addEventListener('layoutChangeComplete', () => {
